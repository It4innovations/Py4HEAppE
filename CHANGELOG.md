...
### Added
- CLI integration tests (live CLI examples) under `tests/` with helper `tests/live_cli_support.py` and a `tests/.env` template to simplify running live examples.
- Updated README with instructions for running CLI tests and environment setup.

### Changed
- Documented test-runner defaults (use Python's `unittest`) and optional `pytest` support.
